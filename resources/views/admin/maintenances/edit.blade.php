    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.maintenances.update", [$maintenance->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crane') ? 'is-invalid' : '' }}" name="crane_id" id="crane_id" required>
                    @foreach($cranes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('crane_id') ? old('crane_id') : $maintenance->crane->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('crane'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id">
                    @foreach($projects as $id => $entry)
                        <option value="{{ $id }}" {{ (old('project_id') ? old('project_id') : $maintenance->project->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('maintenance_data') ? 'is-invalid' : '' }}" type="text" name="maintenance_data" id="maintenance_data" value="{{ old('maintenance_data', $maintenance->maintenance_data) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('conservator') ? 'is-invalid' : '' }}" name="conservator_id" id="conservator_id">
                    @foreach($conservators as $id => $entry)
                        <option value="{{ $id }}" {{ (old('conservator_id') ? old('conservator_id') : $maintenance->conservator->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="discription">{{ trans('cruds.maintenance.fields.discription') }}</label>
                <textarea class="form-control {{ $errors->has('discription') ? 'is-invalid' : '' }}" name="discription" id="discription">{{ old('discription', $maintenance->discription) }}</textarea>
                @if($errors->has('discription'))
                    <div class="invalid-feedback">
                        {{ $errors->first('discription') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.maintenance.fields.discription_helper') }}</span>
            </div>
            <div class="form-group">
