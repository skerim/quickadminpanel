...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.liny.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.linies.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="diameter">{{ trans('cruds.liny.fields.diameter') }}</label>
                <input class="form-control {{ $errors->has('diameter') ? 'is-invalid' : '' }}" type="number" name="diameter" id="diameter" value="{{ old('diameter', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dostawca') ? 'is-invalid' : '' }}" type="text" name="dostawca" id="dostawca" value="{{ old('dostawca', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('length') ? 'is-invalid' : '' }}" type="number" name="length" id="length" value="{{ old('length', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('stan') ? 'is-invalid' : '' }}" type="text" name="stan" id="stan" value="{{ old('stan', '') }}">
                @if($errors->has('stan'))
                    <div class="invalid-feedback">
                        {{ $errors->first('stan') }}
                    </div>
