...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.klimatyzacja.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.klimatyzacjas.update", [$klimatyzacja->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crane') ? 'is-invalid' : '' }}" name="crane_id" id="crane_id" required>
                    @foreach($cranes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('crane_id') ? old('crane_id') : $klimatyzacja->crane->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('crane'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('model') ? 'is-invalid' : '' }}" type="text" name="model" id="model" value="{{ old('model', $klimatyzacja->model) }}" required>
                @if($errors->has('model'))
                    <div class="invalid-feedback">
                        {{ $errors->first('model') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_montazu') ? 'is-invalid' : '' }}" type="text" name="data_montazu" id="data_montazu" value="{{ old('data_montazu', $klimatyzacja->data_montazu) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_konserwacji') ? 'is-invalid' : '' }}" type="text" name="data_konserwacji" id="data_konserwacji" value="{{ old('data_konserwacji', $klimatyzacja->data_konserwacji) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('comments') ? 'is-invalid' : '' }}" type="text" name="comments" id="comments" value="{{ old('comments', $klimatyzacja->comments) }}">
