    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.supports.update", [$support->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crane') ? 'is-invalid' : '' }}" name="crane_id" id="crane_id" required>
                    @foreach($cranes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('crane_id') ? old('crane_id') : $support->crane->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('crane'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('budowa') ? 'is-invalid' : '' }}" type="text" name="budowa" id="budowa" value="{{ old('budowa', $support->budowa) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('region') ? 'is-invalid' : '' }}" type="text" name="region" id="region" value="{{ old('region', $support->region) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kontrahent') ? 'is-invalid' : '' }}" name="kontrahent_id" id="kontrahent_id">
                    @foreach($kontrahents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kontrahent_id') ? old('kontrahent_id') : $support->kontrahent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('transport') ? 'is-invalid' : '' }}" name="transport_id" id="transport_id">
                    @foreach($transports as $id => $entry)
                        <option value="{{ $id }}" {{ (old('transport_id') ? old('transport_id') : $support->transport->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
