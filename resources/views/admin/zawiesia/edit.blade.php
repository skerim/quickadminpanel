...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.zawiesium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.zawiesia.update", [$zawiesium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nr">{{ trans('cruds.zawiesium.fields.nr') }}</label>
                <input class="form-control {{ $errors->has('nr') ? 'is-invalid' : '' }}" type="text" name="nr" id="nr" value="{{ old('nr', $zawiesium->nr) }}">
                @if($errors->has('nr'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('klasa') ? 'is-invalid' : '' }}" type="text" name="klasa" id="klasa" value="{{ old('klasa', $zawiesium->klasa) }}">
                @if($errors->has('klasa'))
                    <div class="invalid-feedback">
                        {{ $errors->first('klasa') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expiration_date') ? 'is-invalid' : '' }}" type="text" name="expiration_date" id="expiration_date" value="{{ old('expiration_date', $zawiesium->expiration_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('load') ? 'is-invalid' : '' }}" type="text" name="load" id="load" value="{{ old('load', $zawiesium->load) }}" required>
                @if($errors->has('load'))
                    <div class="invalid-feedback">
                        {{ $errors->first('load') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('length') ? 'is-invalid' : '' }}" type="number" name="length" id="length" value="{{ old('length', $zawiesium->length) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crane') ? 'is-invalid' : '' }}" name="crane_id" id="crane_id">
                    @foreach($cranes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('crane_id') ? old('crane_id') : $zawiesium->crane->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('crane'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $zawiesium->type) }}">
                @if($errors->has('type'))
                    <div class="invalid-feedback">
                        {{ $errors->first('type') }}
                    </div>
