    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reports.update", [$report->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sn') ? 'is-invalid' : '' }}" name="sn_id" id="sn_id">
                    @foreach($sns as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sn_id') ? old('sn_id') : $report->sn->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sn'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text" name="start" id="start" value="{{ old('start', $report->start) }}">
                @if($errors->has('start'))
                    <div class="invalid-feedback">
                        {{ $errors->first('start') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('stop') ? 'is-invalid' : '' }}" type="text" name="stop" id="stop" value="{{ old('stop', $report->stop) }}">
                @if($errors->has('stop'))
                    <div class="invalid-feedback">
                        {{ $errors->first('stop') }}
                    </div>
